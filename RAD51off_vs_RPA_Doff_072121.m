...
% k_off_RAD51 and k_off_RPA_D. It allows for plotting the growth 
...
k_off_RAD51_Values = linspace(1,10,2);     %kinetic rate constant for RAD51 dissociating from ssDNA
...
k_off_RPA_D_Values = linspace(1,10,2); %kinetic rate constant for RPA-D dissociating from ssDNA

n_RPA = sum([n_A,n_D]);   %calculates total length of RPA molecule
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Heatmap_Parameters = combvec(k_off_RAD51_Values,k_off_RPA_D_Values);   %combination of possible sets of parameters that will generate heatmap (for this instance: k_off_RAD51 and k_off_RPA_D)
...
P1 = Heatmap_Parameters(1,:);  %Temp. variable (for k_off_RAD51) to avoid high, over-necessary communication in parfor of Parameters
P2 = Heatmap_Parameters(2,:);  %Temp. variable (for k_off_RPA_D) to avoid high, over-necessary communication in parfor of Parameters
...
parfor Simulations = 1:(numel(Heatmap_Parameters)/2)
    ...
    k_off_RAD51 = P1(Simulations); %kinetic rate constant for unbinding of RAD51
    k_off_RPA_D = P2(Simulations);    %kinetic rate constant for unbinding of RPA-D
...
[X_Heatmap,Y_Heatmap] = meshgrid(k_off_RAD51_Values,k_off_RPA_D_Values); %generates X and Y values for heatmaps
...
xlabel('RAD51 k_o_f_f');
ylabel('RPA-D k_o_f_f');
