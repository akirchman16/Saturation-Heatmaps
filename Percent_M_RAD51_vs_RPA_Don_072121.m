...
% Percent_M_RAD51 and k_on_RPA_D. It allows for plotting the growth 
...
k_on_RPA_D_Values = linspace(1,10,2);  %kinetic rate constant for RPA-D binding to ssDNA
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Heatmap_Parameters = combvec(Percent_M_RAD51_Values,k_on_RPA_D_Values);   %combination of possible sets of parameters that will generate heatmap (for this instance: Percent_M_RAD51 and k_on_RPA_D)
...
P1 = Heatmap_Parameters(1,:);  %Temp. variable (for Percent_M_RAD51) to avoid high, over-necessary communication in parfor of Parameters
P2 = Heatmap_Parameters(2,:);  %Temp. variable (for k_on_RPA_D) to avoid high, over-necessary communication in parfor of Parameters
...
parfor Simulations = 1:(numel(Heatmap_Parameters)/2)
    ...
    Percent_M_RAD51 = P1(Simulations); %percentage of solution which is monomers
    k_on_RPA_D = P2(Simulations);    %kinetic rate constant for binding of RPA-D
    ...
    RAD51_Dim_BoundAtSpot = zeros(1,N); %array used to record where RAD51 Dimer are bound
...
[X_Heatmap,Y_Heatmap] = meshgrid(Percent_M_RAD51_Values,k_on_RPA_D_Values); %generates X and Y values for heatmaps
