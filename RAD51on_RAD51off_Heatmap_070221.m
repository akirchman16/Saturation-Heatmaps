...
k_on_RAD51_Values = [1,10];     %kinetic rate constant for RAD51 binding to ssDNA
k_off_RAD51_Values = [1,10];    %kinetic rate constant for RAD51 dissociating from ssDNA
...
k_off_RPA_A_Values = 1; %kinetic rate constant for RPA-A dissociating from ssDNA
k_off_RPA_D_Values = 1; %kinetic rate constant for RPA-D dissociating from ssDNA

n_RPA = sum([n_A,n_D]);   %calculates total length of RPA molecule
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Parameters = combvec(L_RAD51_Total_Values,Percent_M_RAD51_Values,w_RAD51_Values,k_on_RAD51_Values,k_off_RAD51_Values,L_RPA_Values,k_on_RPA_A_Values,k_on_RPA_D_Values,k_off_RPA_A_Values,k_off_RPA_D_Values);   %combination of all possible parameter sets
...
for Simulations = 1:(numel(Parameters)/10)
    ...
    w_RAD51 = Parameters(3,Simulations); %RAD51 Cooperativity
    k_on_RAD51 = Parameters(4,Simulations); %Kinetic rate constant for RAD51 binding
    k_off_RAD51 = Parameters(5,Simulations);    %Kinetic rate constant for RAD51 unbinding
    L_RPA = Parameters(6,Simulations);  %RPA Concentration
    k_on_RPA_A = Parameters(7,Simulations); %Kinetic rate constant for RPA-A binding
    k_on_RPA_D = Parameters(8,Simulations); %Kinetic rate constant for RPA-D binding
    k_off_RPA_A = Parameters(9,Simulations);    %Kinetic rate constant for RPA-A unbinding
    k_off_RPA_D = Parameters(10,Simulations);   %Kinetic rate constant for RPA-D unbinding
    ...
    % Equilibrium Testing - Linear Slope & Intercept Method %%%%%%%%%%%%%%%%%%%
        ...
        if Event_Count >= 1000 %only tests for equilibrium after 1000 events have occured
            ...
            RPA_Avg_Saturation(Simulations) = sum(RPA_Equilibrium_Test)/numel(RPA_Equilibrium_Test); %average saturation in last 1/4 of Events (RPA)
            RAD51_Avg_Saturation(Simulations) = sum(RAD51_Equilibrium_Test)/numel(RAD51_Equilibrium_Test);   %average saturation in last 1/4 of Events (RAD51)
...
Z_Heatmap_RAD51 = griddata(Parameters(4,:),Parameters(5,:),RAD51_Avg_Saturation,X_Heatmap(:),Y_Heatmap(:));  %generates heatmap data based on RAD51 saturation
Z_Heatmap_RAD51 = reshape(Z_Heatmap_RAD51,size(X_Heatmap)); %reshapes RAD51 saturation data into appropriately sized matrix
Z_Heatmap_RPA = griddata(Parameters(4,:),Parameters(5,:),RPA_Avg_Saturation,X_Heatmap(:),Y_Heatmap(:));  %generates heatmap data based on RPA saturation
Z_Heatmap_RPA = reshape(Z_Heatmap_RPA,size(X_Heatmap)); %reshapes RPA saturation data into appropriately sized matrix
Z_Heatmap_t = griddata(Parameters(4,:),Parameters(5,:),t_Equilibrium,X_Heatmap(:),Y_Heatmap(:));    %generates heatmap data based on the time to equilibrium
...
imagesc(Parameters(4,:),Parameters(5,:),Z_Heatmap_RAD51);
...
imagesc(Parameters(4,:),Parameters(5,:),Z_Heatmap_RPA);
...
imagesc(Parameters(4,:),Parameters(5,:),Z_Heatmap_t);
