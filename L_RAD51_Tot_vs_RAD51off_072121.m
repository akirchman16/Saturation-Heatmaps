...
% L_RAD51_Total and k_off_RAD51. It allows for plotting the growth 
...
k_off_RAD51_Values = linspace(1,5,2);    %kinetic rate constant for RAD51 dissociating from ssDNA
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Heatmap_Parameters = combvec(L_RAD51_Total_Values,k_off_RAD51_Values);   %combination of possible sets of parameters that will generate heatmap (for this instance: L_RAD51_Total and k_off_RAD51)
...
parfor Simulations = 1:(numel(Heatmap_Parameters)/2)
    ...
    k_off_RAD51 = P2(Simulations);    %RAD51 k_off values
...
[X_Heatmap,Y_Heatmap] = meshgrid(L_RAD51_Total_Values,k_off_RAD51_Values); %generates X and Y values for heatmaps
