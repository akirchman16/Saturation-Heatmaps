...
% Primary difference between this and the normal heatmap codes is that in
% the Total Saturation graph it will graph how much of the saturation is
% RAD51 vs. how much of it is RPA. This will be done using two different
% colors and shading the result between the two colors.
...
k_on_RAD51_Values = 0.1:0.1:2;     %kinetic rate constant for RAD51 binding to ssDNA
k_off_RAD51_Values = 0.1:0.1:2;    %kinetic rate constant for RAD51 dissociating from ssDNA
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Heatmap_Parameters = combvec(k_on_RAD51_Values,k_off_RAD51_Values);   %combination of possible sets of parameters that will generate heatmap (for this instance: k_on_RAD51 and k_off_RAD51)
...
P1 = Heatmap_Parameters(1,:);  %Temp. variable (for k_on_RAD51) to avoid high, over-necessary communication in parfor of Parameters
P2 = Heatmap_Parameters(2,:);  %Temp. variable (for k_off_RAD51) to avoid high, over-necessary communication in parfor of Parameters
...
CustomMapBot = [linspace(MidColor(1),MaxColor(1),100*(numel(Z_Heatmap_Shade)-index))',linspace(MidColor(2),MaxColor(2),100*(numel(Z_Heatmap_Shade)-index))',linspace(MidColor(3),MaxColor(2),100*(numel(Z_Heatmap_Shade)-index))'];
CustM = [CustomMapTop;CustomMapBot];

colormap(figure(1),CustM);    %colormap for the shaded plot

figure(1);
imagesc(Heatmap_Parameters(1,:),Heatmap_Parameters(2,:),Z_Heatmap_Shade);
title('%RAD51 Saturation (of Total Sat.)');
xlabel('RAD51 k_o_n');
