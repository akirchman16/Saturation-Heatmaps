...
k_on_RPA_D_Values = 1:1:100;  %kinetic rate constant for RPA-D binding to ssDNA
...
w = waitbar(0,['Running ', num2str(numel(Parameters)/10), ' Simulations...']);  %generates a progress bar for the simulations
...
parforWaitbar(w, numel(Parameters)/10); %defines inputs for parforWaitbar function
...
parfor Simulations = 1:(numel(Parameters)/10)
    ...
    w_RAD51 = P3(Simulations); %RAD51 Cooperativity
    k_on_RAD51 = P4(Simulations); %Kinetic rate constant for RAD51 binding
    k_off_RAD51 = P5(Simulations);    %Kinetic rate constant for RAD51 unbinding
    L_RPA = P6(Simulations);  %RPA Concentration
    k_on_RPA_A = P7(Simulations); %Kinetic rate constant for RPA-A binding
    k_on_RPA_D = P8(Simulations); %Kinetic rate constant for RPA-D binding
    k_off_RPA_A = P9(Simulations);    %Kinetic rate constant for RPA-A unbinding
    k_off_RPA_D = P10(Simulations);   %Kinetic rate constant for RPA-D unbinding
